# Spain Environment — eu-south-2 (GDPR)

Deploys the complete payment platform infrastructure for Spain. All data stays within the `eu-south-2` AWS region to comply with GDPR.
...
├── ACM certificate        — TLS for api-es.aws.lacloaca.com
└── Secrets Manager        — API_SECRET_KEY storage
```

## State Backend

S3 bucket `aabella-terraform-backends`, key `spain/terraform.tfstate`, region `eu-south-2`.
...
|------|-------------|------|---------|----------|
| aws_region | AWS region for this environment | `string` | `"eu-south-2"` | no |
| project_name | Project name prefix for all resources | `string` | `"pluxee-spain"` | no |
| environment | Environment name | `string` | `"production"` | no |
| region_name | Human-readable region identifier | `string` | `"spain"` | no |
| data_residency | Data residency classification (EU, MX, etc.) | `string` | `"EU"` | no |
| compliance | Compliance framework | `string` | `"GDPR"` | no |
| vpc_cidr | CIDR block for the VPC | `string` | `"10.10.0.0/16"` | no |
| private_subnets | Private subnet CIDR blocks | `list(string)` | `["10.10.1.0/24", "10.10.2.0/24", "10.10.3.0/24"]` | no |
| public_subnets | Public subnet CIDR blocks | `list(string)` | `["10.10.11.0/24", "10.10.12.0/24", "10.10.13.0/24"]` | no |
| database_subnets | Database subnet CIDR blocks | `list(string)` | `["10.10.21.0/24", "10.10.22.0/24", "10.10.23.0/24"]` | no |
...
| argocd_source_cidr | Additional CIDR added to EKS endpoint allowlist. Not used for Spain (ArgoCD runs in-cluster) but kept for structural consistency with Mexico. | `string` | `""` | no |
...
| app_domain | FQDN for the application Ingress | `string` | `"api-es.aws.lacloaca.com"` | no |
